...
└───┬──────────────┬────────────────┬────────────┘
    │              │                │
┌───▼──────┐  ┌────▼──────┐  ┌──────▼─────────────────────┐
│ MySQL 8  │  │ Qdrant    │  │ 模型服务（RTX 5000 16GB）     │
│ 会话记录  │  │ 向量+元数据│  │ · llama.cpp: Qwen3-8B GGUF  │
│ 文档注册表│  │ 过滤(Docker)│  │   Q4_K_M（已部署）           │
└──────────┘  └───────────┘  │ · 嵌入: BGE-M3 (FlagEmbedding)│
...
### 3.1 显存预算（RTX 5000 16GB，部署实测后核算）

| 模型 | 显存 |
|---|---|
| Qwen3-8B GGUF Q4_K_M（llama.cpp） | ~5GB |
| BGE-M3 嵌入服务（fp16） | ~1.3GB |
| bge-reranker-v2-m3（fp16） | ~1.2GB |
| KV cache + 运行余量 | ~6-8GB |
| **合计** | **~14-15GB** |

余量 1~2GB。llama.cpp `--ctx-size` 与并发数受限（试点单用户串行无压力）；重排模型仅在检索阶段调用，可考虑冷加载策略（如显存不足时后置加载）。
